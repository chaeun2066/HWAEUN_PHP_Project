  ...
  <section>
    <div class="main_content">
      ...
      <div class="center">
        <div>
          <ul class="somenu">
            <li><a href="../notice/notice_list.php" id="title_somenu">Notice</a></li>
            <li><a href="../notice/notice_list.php" id="li_somenu">공지사항</a></li>
            <li><a href="./event_list.php" id="li_somenu_first">이벤트</a></li>
          </ul>
        </div>
        <div>
          <h2>이벤트</h2>
...
<?php
            ...
            $sql = "select count(*) from event order by num desc";
            ...
            $sql = "select * from event order by num desc LIMIT $start, $scale";
            $result = mysqli_query($con, $sql);
            $i = 0;
            ...
            for($i=0; $i< count($list); $i++){
              $file_image = (!empty($list[$i]['file_name']))?"<i class=\"fa-solid fa-floppy-disk\"></i>" :" ";
              ...
              if (!empty($list[$i]['file_name'])) {
                ...
                if ($image_width > 330 ) $image_width = 330;
                if ($image_height > 180 ) $image_height = 180;
                $file_copied_0 = $list[$i]['file_copied'];
              }
?>
              <li class="item">
                <span class="subject">
                  <a href="event_view.php?num=<?= $list[$i]['num'] ?>&page=<?= $page ?>">
<?php
...
?>
          ...
          <ul id="page_num">
<?php
            $url = "event_list.php?page=1";
            echo get_paging(2, $page, $total_page, $url);
?>
          </ul>
          <ul class="buttons">
            <li><button onclick="location.href='event_list.php'" class="green_button">목록</button></li>
            <li>
<?php 
            $id = isset($_SESSION['id']) ? $_SESSION['id'] : "";
            if($id == 'admin'){
              echo "<li><button onclick=\"location.href='event_form.php'\" class=\"green_button\">글쓰기</button></li>";
            }
?>
            </li>
          </ul>  
